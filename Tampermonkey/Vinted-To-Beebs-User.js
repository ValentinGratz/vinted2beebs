    ...
    function showPanel(vinted, beebs) {
        ...
        panel.querySelector('#vbb-dl-all').onclick = () => {
            vinted.photos.forEach((url,i) => {
                setTimeout(() => {
                    if (typeof GM_download !== 'undefined') GM_download(url, `beebs-${beebs.title.substring(0,10)}-${i+1}.jpg`);
        ...
        panel.querySelector('#vbb-open-beebs').onclick = () => {
            ...
            if (typeof GM_setClipboard !== 'undefined') GM_setClipboard(fullText);
            else navigator.clipboard.writeText(fullText);
            window.open('https://www.beebs.app/', '_blank');
            if (typeof GM_notification !== 'undefined') GM_notification({title:'Vinted → Beebs', text:'Tout copié ! Colle dans l\'app Beebs'});
        };
    }
